...
namespace V2RayW
{
    ...
    static class Program
    {
        ...
        //subscriptions data
        public static List<SubsProfile> subsProfiles = new List<SubsProfile>();
        public static DataTable dt = new DataTable();
        public static BindingSource bs = new BindingSource();
        public static Dictionary<int, SubsProfile> groupSubsMap = new Dictionary<int, SubsProfile>();
        ...
        public static async void configurationDidChange()
        {
            ...
            if(coreLoaded)
            {
                ...
                else 
                {
                    ...
                    MessageBox.Show(I18N.GetValue("No available servers!") + selectedServerIndex, "Info", MessageBoxButtons.OK, MessageBoxIcon.Information);
                }
                updateSystemProxy();
            }
            mainForm.updateMenu();
        ...
        private static void initSubsData()
        {
            dt.Columns.Add("remark");
            dt.Columns.Add("url");
            //dt.Columns.Add("groupId");
            DataRow dr0 = Program.dt.NewRow();
            dr0["remark"] = "-all-";
            dr0["url"] = "///:all";
            //dr0["groupId"] = 0;
            dt.Rows.Add(dr0);
            subsProfiles.Add(new SubsProfile());
            readSubsData();
            foreach (SubsProfile subsProfile in subsProfiles[0].SubsRefList)
            {
                subsProfiles.Add(subsProfile);
                groupSubsMap[subsProfile.groupId] = subsProfile;
                DataRow dr = Program.dt.NewRow();
                dr["remark"] = subsProfile.remark;
                dr["url"] = subsProfile.url;
                //dr["groupId"] = subsProfile.groupId;
                dt.Rows.Add(dr);
            }
            bs.DataSource = dt;
        }

        private static void saveSubsData()
        {
        ...
        private static void readSubsData()
        {
            string filepath = string.Format("{0}\\{1}.json", SubsSaveDir, Utils.MD5Sum("-"));
            if (File.Exists(filepath))
            {
                SubsProfile tmp = IOSerialize.readingJson(filepath);
                subsProfiles[0] = tmp;
            }
        }
    }
